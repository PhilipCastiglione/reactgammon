...
const Board = (props) => {
  ...
  return (
    <section className="board">
      <EndSlot
        player={1}
        playerOneCheckers={props.checkers[0].filter(c => {return c === -1})}
        playerTwoCheckers={props.checkers[1].filter(c => {return c === -1})}
      />
      {slots}
      <EndSlot
        player={0}
        playerOneCheckers={props.checkers[0].filter(c => {return c === 24})}
        playerTwoCheckers={props.checkers[1].filter(c => {return c === 24})}
      />
    </section>
  )
}
